...
const History = () => {
  ...
  const getScoreMode = (item) =>
    item?.score_mode === "points" ? "points" : "percent";
  const formatScore = (item) =>
    `${item?.score ?? 0}${getScoreMode(item) === "points" ? " pts" : "%"}`;
  const getScoreColorClass = (item) => {
    const maturityLevel = item?.maturity_level;
    if (maturityLevel === "Inicial" || maturityLevel === "Em Desenvolvimento") {
      return "text-accent-warning";
    }
    return "text-accent-success";
  };
  const getScoreBadgeClass = (item) => {
    const maturityLevel = item?.maturity_level;
    if (maturityLevel === "Inicial" || maturityLevel === "Em Desenvolvimento") {
  ...
  const filteredHistory = historyArray.filter((item) => {
    ...
    const matchesSearch =
      ...
      (item.userInfo?.vendedor?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      ) ||
      (item.userInfo?.etn?.toLowerCase() || "").includes(
        searchTerm.toLowerCase(),
      );
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 animate-fadeIn">
        ...
        <p className="text-text-secondary mb-8">
          Você ainda não realizou nenhum diagnóstico de maturidade.
        </p>
        <Link
          to="/diagnostico"
  ...
  return (
    <div className="max-w-4xl mx-auto animate-fadeIn pb-20">
      ...
      <div className="flex flex-col gap-4">
        {filteredHistory.length === 0 ? (
          ...
          filteredHistory.map((item, idx) => (
            <div
              key={item.id || idx}
              className="bg-white p-6 rounded-xl border border-border-color shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
            >
              <div
              ...
              <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                ...
                >
                  ...
                  <div
                    className={`text-2xl font-bold ${getScoreColorClass(item)}`}
                  >
                    {formatScore(item)}
                  </div>
                </div>
      ...
      {selectedItem && (
        ...
        >
          <div
            className="bg-white rounded-xl max-w-lg w-full p-6 shadow-2xl overflow-hidden"
            ...
            <div className="flex justify-between items-start mb-6 border-b border-border-color pb-4">
              <div>
                ...
                <p className="text-sm text-text-secondary">
                  {new Date(selectedItem.date).toLocaleDateString("pt-BR")} às{" "}
                  {new Date(selectedItem.date).toLocaleTimeString("pt-BR")}
                </p>
              </div>
              <div
                className={`px-3 py-1 rounded-full text-sm font-bold ${getScoreBadgeClass(selectedItem)}`}
              >
                Score: {formatScore(selectedItem)}
              </div>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p
                    ...
                    title={selectedItem.userInfo?.email}
                  >
                    {selectedItem.userInfo?.email || "-"}
                  </p>
                </div>
                <div>
                  <label className="text-xs font-bold text-text-secondary uppercase block mb-1">
                  ...
                  <p className="font-medium">
                    {selectedItem.userInfo?.vendedor || "-"}
                  </p>
                </div>
                <div>
                  <label className="text-xs font-bold text-text-secondary uppercase block mb-1">
      ...
      {deleteModal.isOpen && (
        ...
        >
          <div
            className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl overflow-hidden"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex flex-col items-center text-center mb-6">
              ...
              <p className="text-xs text-text-secondary mt-2 font-bold uppercase text-accent-danger">
